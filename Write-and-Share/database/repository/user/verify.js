...
async function verify(db, credentials, model){
    ...
    var emailValidityCheck = await checkValidEmail(db,credentials)
    return new Promise(function(resolve,reject){
        if(emailValidityCheck.hasEmail){
            ...
            db.query(sql,credentials.email,async function(error,result,fields){
                try {
                    ...
                    if(isValid){
                        ...
                        model.token = createJwt(getJwtHeader("HS256","JWT"),getJwtPayload(result[0].id,result[0].name,result[0].email,result[0].username,result[0].phone,result[0].id === 1 ? true : false))
                        resolve(model)
                    }
                    else{
